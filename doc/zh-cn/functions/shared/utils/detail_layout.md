...
- **备注：** 窗格取代手机上显示的底部面板，所以封顶 380——412 dp 手机上底部面板的内容宽度（412 减 2 × 16）——详情的排版永远不比手机上更宽。280 下限让路由行的两行副标题和「编辑服务」/「添加访问方式」按钮在中文和英文下都保持在一行。0.3 的比例让画布在 934 dp 及以上的窗口中占 70 %；低于此宽度时 280 的下限生效，画布在 600 dp 分栏下限时保留 319，Z Fold 8 横屏（933）时 652。`test/detail_layout_test.dart` 钉住命名宽度、600–3000 dp 范围内相对手机底部面板的封顶，以及 0.3 的比例。

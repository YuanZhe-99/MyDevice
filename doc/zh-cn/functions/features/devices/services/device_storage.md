...
| `_dataFileName` | 静态常量（私有） | B | `'device_data.json'`，设备列表的文件名。 |
| `_configFileName` | 静态常量（私有） | B | `'storage_config.json'`，本地设置文件名。 |
...
| [`setStoragePath`](#setstoragepath) | 静态方法 | A | 更改存储位置，并把整个存储文件夹移过去。 |
...
行数（35）比 `grep -c '/// Purpose:' device_storage.dart`（30）多五。30 个静态方法（包括八个列表列数访问器中的每一个）各有自己的行和自己的 `Purpose:` 块。多出的五行是 `DeviceStorage` 类本身、私有静态常量 `_dataFileName` 和 `_configFileName`，以及私有静态字段 `_customPath` 和 `_configLoaded`，它们带普通 `///` 描述或没有注释，因每个声明都出现在表中而列出。Tier A：22 行。

## 文档

### `static Future<Directory> _getDefaultAppDir()` <a id="_getdefaultappdir"></a>
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 31 行）。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 46 行）。
- **用途：** 解析 `storage_config.json` 文件路径，无论配置任何自定义存储路径它总是住在*默认*应用目录。
- **输入：** 无。
- **返回：** `Future<File>`。
- **副作用：** 无（不创建文件）。
- **算法：** 把 `_getDefaultAppDir()` 路径与 `_configFileName` 连接。
- **用法：** 被 [`_loadCustomPath`](#_loadcustompath)、[`_readConfigFromDefault`](#_readconfigfromdefault) 和 [`_writeConfigToDefault`](#_writeconfigtodefault) 调用。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 57 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 77 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 95 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 120 行）。
- **用途：** 更改应用存储位置，并把旧存储文件夹中的一切移到新位置。
- **输入：** `newPath` — 新自定义路径，或 `null`/空回退默认目录。
- **返回：** `Future<bool>` — 只有异常逸出时（例如读写 `storage_config.json` 时）为 `false`；留下部分条目未移动的迁移仍返回 `true`。
- **副作用：** 设置 `_customPath`；把 `storagePath` 持久化到默认目录的 `storage_config.json`；经 `myapps_data` 的 `migrateStorageContents`（`packages/myapps_data/lib/src/storage/storage_migration.dart`）把旧文件夹内容移入新文件夹。
- **算法：** 1. 把当前目录捕获为 `oldDir`。2. 设 `_customPath = newPath`，并经 [`_readConfigFromDefault`](#_readconfigfromdefault) / [`_writeConfigToDefault`](#_writeconfigtodefault) 持久化进 `storage_config.json`，`newPath` 为 null 或空时移除该键。3. 经 `getAppDir()`（会创建目录）解析 `newDir`；其路径等于 `oldDir` 时返回 `true`（无需移动）。4. `await migrateStorageContents(from: oldDir, to: newDir)`：除 `storage_config.json` 外的每个顶层文件和目录逐文件复制进 `newDir`，每个原件在复制后删除；源目录只在变空后才移除。返回的未能移动路径列表被丢弃。5. 返回 `true`；整个方法体位于返回 `false` 的 `try`/`catch` 中。
- **用法：**
  ```dart
  final ok = await DeviceStorage.setStoragePath(pathToSet);
  ```
  （来自 `settings_page.dart` 的"更改存储位置"流程）
- **备注：** 移动覆盖整个文件夹而非枚举列表：全部四个数据文件、`images/`、`.sync_base/`、含 `backups/blobs/` 的 `backups/` 以及 `webdav_config.json`，因此以后新增的数据文件会自动随之移动。它取代了按目录复制的旧做法：旧做法把 `backups/blobs/` 留在原处（恢复的备份丢失其图像），并完全漏掉 `.sync_base/`，使下一次同步把其他设备已删除的记录当作新的本地记录并使其复活。目标位置已存在的文件胜出，其源副本留在原处，因此不会凭对哪个副本更新的猜测丢弃任何东西。`storage_config.json` 留在平台默认目录，因为它保存的正是自定义路径本身。其结果是该文件中的其他键（主题、语言区域、默认货币、列表列数偏好等）不会被带过去：[`readConfig`](#readconfig) 对照新目录解析，而新目录在某个设置再次写入之前并不存在 `storage_config.json`。

### `static Future<Map<String, dynamic>> _readConfigFromDefault()` <a id="_readconfigfromdefault"></a>
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 153 行）。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 167 行）。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 179 行）。
- **用途：** 解析*当前*应用目录内命名文件（尊重任何自定义存储路径）。
- **输入：** `name` — 裸文件名（如 `device_data.json`）。
- **返回：** `Future<File>`。
- **副作用：** 除 `getAppDir()` 的目录创建副作用外无。
- **算法：** `File(p.join((await getAppDir()).path, name))`。
- **用法：** 被 [`load`](#load)、[`save`](#save)、[`readConfig`](#readconfig) 和 [`writeConfig`](#writeconfig) 调用。[`DeviceExchangeRateService._getFile`](exchange_rate_service.md#_getfile) 不调用它；它直接把自己的文件名拼接到 [`getAppDir`](#getappdir) 上。
- **备注：** 无。

### `static Future<DeviceData> load()` <a id="load"></a>
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 191 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 205 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 218 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 239 行）。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 251 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 306 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 318 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 328 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 343 行）。
...
- **种类：** 静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 353 行）。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 371 行）。
- **用途：** 读取一个列表页的列数偏好。
...
- **种类：** 私有静态方法。
- **来源：** `lib/features/devices/services/device_storage.dart`（第 386 行）。
- **用途：** 持久化一个列表页的列数偏好。

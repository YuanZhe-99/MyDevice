...
| [`_showStoragePathDialog`](#_showstoragepathdialog) | 方法（`_SettingsPageState`） | A | 让用户设置或重置自定义存储路径，并报告移动留下了什么。 |
...
- **来源：** `lib/features/settings/views/settings_page.dart`（第 68 行）
...
- **备注：** 对应 `dispose()`（第 96 行）调用 `AutoSyncService.instance.removeOnStatusChanged(_refreshSyncStatus)` 避免泄漏监听器。

### `String? _webDavStatusText(AppLocalizations l10n)` <a id="_webdavstatustext"></a>
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 106 行）
...
- **算法：**
  ...
  3. 否则返回 `null`（尚无同步运行）。
- **用法：** `build` 顶部 `final webDavStatus = _webDavStatusText(l10n);`（`lib/features/settings/views/settings_page.dart`，第 660 行），显示为 WebDAV 同步块副标题，`lastError` 已设时用 error 色样式。
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 124 行）
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 134 行）
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 146 行）
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 273 行）
...
- **用法：** `build` 中"Export data"块的 `onTap: _exportData`（`lib/features/settings/views/settings_page.dart`，第 840 行）。
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 332 行）
...
- **用法：** `build` 中"Import data"块的 `onTap: _importData`（`lib/features/settings/views/settings_page.dart`，第 845 行）。
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 373 行）
...
- **用法：** `build` 中"Data Migration"块的 `onTap: _openDataFolder`（`lib/features/settings/views/settings_page.dart`，第 865 行），只在 `_isDesktop` 时显示。
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 394 行）
- **用途：** 让用户输入自定义存储目录或重置为默认，应用它，并报告结果。
- **输入：** `context`。
- **返回：** `Future<void>`。
- **副作用：** 显示带文本字段的对话框；调用 `DeviceStorage.setStoragePath`；重载显示的路径；显示失败 snackbar、未移动条目对话框或成功 snackbar。
- **算法：**
  ...
  4. Await `DeviceStorage.setStoragePath(pathToSet)`，它返回一个 [`StoragePathResult`](../../devices/services/device_storage.md#storagepathresult-new)。
  5. `result.saved` 为 false（路径未能记录，因此什么都没变）时显示 `settingsStoragePathFailed` snackbar 并返回。
  6. 否则经 [`_loadStoragePath`](#_loadstoragepath) 重载。`result.unmoved` 非空时显示一个 `AlertDialog`（键 `storage-unmoved-dialog`，警告图标），标题为 `settingsStoragePathUnmovedTitle`，正文为 `settingsStoragePathUnmovedBody(result.from)`，其后把每个未移动的相对路径列为等宽 `SelectableText`，只有一个确定按钮；然后返回。
  7. 否则显示 `settingsResetDefaultLocation` 或 `settingsStoragePathUpdated`。
- **用法：** `build` 中"Storage Location"块的 `onTap: () => _showStoragePathDialog(context)`（`lib/features/settings/views/settings_page.dart`，第 858 行），只在 `_isDesktop` 时显示。
- **备注：** 用本地 `context` 参数的 `.mounted`（`context.mounted`，第 441、449 和 482 行）而非 State 自己 `mounted`，因为此方法显式接收 `context`。未移动条目对话框之所以存在，是因为留在旧文件夹中的条目在新位置无法被应用读取；用户必须手动移动它。

### `Future<void> _loadTraySettings()` <a id="_loadtraysettings"></a>
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 499 行）
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 517 行）
- **用途：** 查询操作系统级启动时启动注册状态。
- **输入：** 无。
- **返回：** `Future<void>`。
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 533 行）
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 550 行）
...
- **用法：** `build` 中"API Server"设置块的 `onTap: _apiEnabled ? _showApiSettingsDialog : null`（`lib/features/settings/views/settings_page.dart`，第 940 行）——只在 API 服务器启用时可点击。
...
- **种类：** `_SettingsPageState` 的方法
- **来源：** `lib/features/settings/views/settings_page.dart`（第 634 行）
...
- **用法：** `build` 中"Refresh Exchange Rates"块的 `onTap: _refreshExchangeRates`（`lib/features/settings/views/settings_page.dart`，第 805 行）。
- **备注：** 无。

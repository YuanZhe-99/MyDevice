...
| 4 | Layout engine: row stride, domain alignment, crossing sweep, device containers | done | One commit, not one per item: items 1–4 share `_placeNodes` and were verified together. Deviations: `ServiceTopologyLayoutOptions` also has `crossingSweeps` (default 4; 0 = the row order, which the tests compare against) and defaults `groupByDevice` to **false** so a bare `build` routes every edge — the page passes true and has the toggle (session only, not persisted). `rowGap` = 36. The crossing counter is public (`countCrossings`) and counts per edge pair across every rank line, long edges interpolated, so a crossing exactly on a rank line still counts. Only devices hosting a service are grouped (a router named only by a hop stays a card). The header is a tab at most `nodeWidth` wide on the container's top-left, not a full-width strip — a full-width strip blocked every edge entering from above; members are re-placed at their row targets inside the container. The container pass is constructive (per-rank floors plus a running shift), so it has no fallback. Item 5 (fan-out bundling) skipped. Item 6 found the pre-existing router far over budget (a 43-node graph took 20–30 s), so the router got result-identical speed-ups: `_RoutedSegments` indexes routed segments by axis, `_routeBetween` caches each grid step's cost per search and uses typed arrays — the 43-node graph now takes ~1 s flat / ~0.2 s grouped, the 61-node test graph ~3.4 s / ~0.4 s; `compute()` not attempted. Device cards and headers now show the localized category instead of the raw enum name. |
| 5 | Topology as a launchpad; editor parity; small list polish | done | Deviations: the route-opening rule uses the lossless round trip (`serviceRouteOpensGuided` = `fromRoute != null`, which `detectServiceAccessPattern` already equals), so the guided page never hands an opened route straight over. A draft's `initialDeviceId` is a UI hint excluded from `==`; the device's only service is preselected, several are *suggested first* in the source picker rather than the picker being filtered. `forNode` also covers endpoint chips (a relay's own port, i.e. the FRP ingress chip, gives the relay draft with that ingress) and returns null for relays and remote entries; relays are recognized by `serviceRelayPatternFor` (a VPS service named/templated `pangolin` or `frp`), stricter than `isFrpLikeService`. Added the handover's follow-up: the topology owns a copy of the inventory, awaits every editor it opens and redraws through a new `reload` callback; `onEditService`/`onEditRoute` now return `Future<void>`. The guided page also prefills the FRP public host for a draft that arrives with its relay. |

...
- **节点详情。** 在手机上，点击还会打开一个底部面板：节点本身、它的设备和服务（带「编辑服务」和该节点的访问路径操作，见 [从拓扑开始](#adding-an-access-path)），以及它的路由，每条都能打开各自的编辑器。这些编辑器中任何一个关闭后，拓扑都会重绘。在分栏窗口上（[自适应布局](../adaptive-layout.md#detail-pages-two-panes)），同样的详情放在画布旁的窗格中；在那里，点击路由行会把高亮收窄到这一条路由（再点一次恢复全部），行上的编辑按钮则打开编辑器。
...
角色、车道、设备类别、路由方法和访问级别标签处处本地化——远端中继服务的副标题以界面语言显示"{method} 服务"（如"FRP 服务"），而非图构建器记录的英文；屏幕阅读器会读出每个节点的标签、角色和车道。
...
**打开已保存的路由**——无论从路由卡片、路由分组、总览还是拓扑详情——都遵循同一规则：引导式页面能在不改动、不丢失任何内容的前提下编辑该路由时（`serviceRouteOpensGuided`），路由在引导式页面中打开，否则在高级编辑器中打开。因此由引导式页面保存的路由会在那里重新打开。

**从拓扑开始。** 节点详情提供适合该节点的访问路径，并以预填好的内容打开引导式页面：

| 节点 | 操作 | 预填内容 |
|---|---|---|
| 服务 | 从这里添加访问路径 | 源服务 |
| VPS 上的中继服务（FRP、Pangolin） | 通过此中继公开服务 | 模式、公网可达范围、中继，FRP 还有其默认入口 |
| 端点 chip | 从这里添加访问路径 | 源服务和端点（对中继自身的端口，则为带该入口的中继草稿） |
| 域名 | 为此目标添加其他服务 | 目标；到它的每条路由模式相同时还有该模式 |
| 设备 | 为此设备上的服务添加访问路径 | 设备唯一的服务作为源；有多个服务时，源选择器把该设备的服务排在前面 |

拓扑打开的任何编辑器关闭后，拓扑会重新读取清单并重绘，因此新建或编辑过的路径无需重新打开拓扑即可显示。路由视图的卡片显示每条路由的方法图标，并在路径下方显示其访问级别和车道。

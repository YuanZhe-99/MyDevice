...
/// The device, network and dataset lists rely on `ListView.builder`
/// virtualization that a pre-built grid would throw away, and the services
/// views build their cards as children of an outer `ListView`, where a nested
/// scrollable would need `shrinkWrap`. Feeding this from a builder over
/// [listRowCount] rows keeps both properties and yields left-to-right,
/// top-to-bottom order. Short final rows are padded with empty cells so the
...
/// their single-column tile in something else — the device list's

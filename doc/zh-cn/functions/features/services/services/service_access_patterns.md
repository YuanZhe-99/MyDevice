...
| [`forNode`](#fornode) | 静态方法（`ServiceAccessDraft`） | A | 拓扑节点的「添加访问路径」操作起步所用的草稿。 |
| `relayDraft`（嵌套于 `forNode`） | 本地函数 | B | 中继服务的中继草稿，优先使用给定的 FRP 入口。 |
...
| [`serviceRelayPatternFor`](#servicerelaypatternfor) | 顶层函数 | A | VPS 上具名的中继服务取 FRP 或 Pangolin，否则为 null。 |
| [`serviceRouteOpensGuided`](#serviceroutesopensguided) | 顶层函数 | A | 保存的路由在引导式页面还是高级编辑器中打开。 |
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 255 行。
- **用途：** 创建替换或清除所选字段的副本。
...
- **备注：** `routeId`、`extraJson`、`baseHops` 和 `initialDeviceId` 总是原样带过，因此无论用户改了什么，编辑都保持其身份。

### `ServiceRoute toRoute({required List<ServiceNode> services})` <a id="toroute"></a>
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 333 行。
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 375 行。
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 402 行。
- **用途：** 构建模式自身的跳。
- **输入：** `byId` — 按 id 索引的服务；`base` — 该位置上被编辑的跳。
- **返回：** `ServiceRouteHop`。**副作用：** 无。
- **算法：**
...
- **种类：** `ServiceAccessDraft` 的静态方法。**来源：** 第 483 行。
- **用途：** 为引导式页面把保存的路由读回草稿。
...
- **用法：** [`serviceRouteOpensGuided`](#serviceroutesopensguided) 用它决定保存的路由在引导式页面还是高级编辑器中打开；引导式页面用它读取所打开的路由。
...
- **种类：** `ServiceAccessDraft` 的静态方法。**来源：** 第 568 行。
- **用途：** 构建拓扑节点的「添加访问路径」操作起步所用的草稿。
- **输入：** `node`；`services`、`routes`、`devices` — 构建拓扑图所用的清单。
- **返回：** 草稿；没有此类操作的节点为 null。**副作用：** 无。
- **算法：** 按节点类型：
  - **device** — `ServiceAccessDraft(initialDeviceId: node.deviceId)`。
  - **service** — [`serviceRelayPatternFor`](#servicerelaypatternfor) 把该服务认定为中继时，取本地函数 `relayDraft`：该模式、其默认（公网）可达范围、中继，FRP 还有默认入口（`serviceDefaultIngressEndpoint`）；否则为 `ServiceAccessDraft(sourceServiceId: …)`。
  - **endpoint** — 中继的端口（角色 `remoteService`，例如 FRP 入口 chip）得到以该端点为入口的中继草稿；其他任何端点得到设置了 `sourceEndpointId` 的源草稿。
  - **domain** — 节点的完整目标（`detail`，否则 `label`）作为唯一目标；经过该节点的每条路由检测出的模式（[`detectServiceAccessPattern`](#detectserviceaccesspattern)）全部一致时取该模式及其默认可达范围，否则为直连。
  - **relay**、**remoteEntry** — null；服务或端点已不存在的节点同样为 null。
- **用法：** 拓扑详情中的操作（[`service_topology_page.md`](../views/service_topology_page.md)）。
- **备注：** 中继服务在拓扑上同时提供两种操作——从中继本身添加访问路径（普通源草稿，由页面构建）和通过它公开另一个服务（本草稿）。测试见 `test/service_access_patterns_test.dart`（"drafts from topology nodes"）。

### `bool operator ==(Object other)` <a id="equals"></a>
- **种类：** `ServiceAccessDraft` 的运算符。**来源：** 第 650 行。
- **用途：** 按表单内容比较草稿。
- **输入：** `other`。**返回：** `bool`。**副作用：** 无。
- **算法：** 比较每个内容字段；`targets` 和 `extraJson` 经 `_jsonEquals` 深比较。
- **用法：** 往返测试把 `fromRoute(draft.toRoute(...))` 与原草稿比较。
- **备注：** `routeId` 和 `baseHops` 是身份元数据，`initialDeviceId` 是 UI 提示；它们都不参与比较。
...
- **用法：** 高级编辑器的"引导式编辑器"操作，以及 [`forNode`](#fornode) 的域名分支。
...
- **种类：** 顶层函数。**来源：** 第 869 行。
- **用途：** 指出 VPS 服务向其他服务提供的中继模式。
- **输入：** `service`；`devices` — 用于找到它的设备。**返回：** `pangolin`、`frp` 或 null。**副作用：** 无。
- **算法：** 服务所在设备的类别不是 `vps` 时为 null；否则小写的名称、模板 id 或图标包含 "pangolin" 时为 `pangolin`，包含 "frp" 时为 `frp`，否则为 null。
- **用法：** [`forNode`](#fornode)，用于服务节点和中继的端点 chip。
- **备注：** 比 [`isFrpLikeService`](#isfrplikeservice) 更严格：仅有 kind `tunnel` 不算，因为 Cloudflare 和 Tailscale 隧道不是用来公开其他服务的中继。

### `bool serviceRouteOpensGuided(ServiceRoute route, List<ServiceNode> services)` <a id="serviceroutesopensguided"></a>
- **种类：** 顶层函数。**来源：** 第 896 行。
- **用途：** 决定保存的路由在哪个编辑器中打开。
- **输入：** `route`、`services`。**返回：** 引导式页面为 `true`，高级编辑器为 `false`。**副作用：** 无。
- **算法：** `ServiceAccessDraft.fromRoute(route, services) != null`。
- **用法：** [`service_list_page.md`](../views/service_list_page.md) 中的 `_editRoute`，所有打开已保存路由的地方都经过它——路由卡片、路由分组、总览和拓扑详情。
- **备注：** 采用无损往返而非更宽松的分类，意味着引导式页面从不需要把刚打开的路由直接转交给高级编辑器。

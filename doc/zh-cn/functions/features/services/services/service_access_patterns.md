...
| [`isFrpLikeService`](#isfrplikeservice) | 顶层函数 | A | FRP 中继启发式，从快速访问对话框移到这里。 |
...
- **种类：** `ServiceAccessPattern` 的 getter。**来源：** 第 43 行。
...
- **种类：** `ServiceReachability` 的 getter。**来源：** 第 124 行。
- **用途：** 返回可达范围保存的路由访问级别。
...
- **种类：** `ServiceReachability` 的 getter。**来源：** 第 136 行。
- **用途：** 返回可达范围固定的拓扑车道。
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 247 行。
- **用途：** 创建替换或清除所选字段的副本。
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 324 行。
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 366 行。
...
- **种类：** `ServiceAccessDraft` 的方法。**来源：** 第 393 行。
- **用途：** 构建模式自身的跳。
- **输入：** `byId` — 按 id 索引的服务；`base` — 该位置上被编辑的跳。
- **返回：** `ServiceRouteHop`。**副作用：** 无。
- **算法：**
...
- **种类：** `ServiceAccessDraft` 的静态方法。**来源：** 第 474 行。
- **用途：** 为引导式页面把保存的路由读回草稿。
...
- **种类：** `ServiceAccessDraft` 的运算符。**来源：** 第 550 行。
- **用途：** 按表单内容比较草稿。
...
- **用法：** 为端口映射跳排序中继候选。
- **备注：** 从快速访问对话框的私有 `_isFrpLikeService` 原样移来。

...
| `_buildSourceFields` | 方法（组件辅助） | B | 源/端点选择器、访问级别（本地化标签）、目标字段和预览卡片。 |
...
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 46 行）
...
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 87 行，从 `initState` 第 65 行调用）
...
- **种类：** `_ServiceRouteEditPageState` 的方法
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 127 行）
...
- **种类：** `_ServiceRouteEditPageState` 的方法
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 160 行）
- **用途：** 让用户确认，然后删除被编辑路由。
- **输入：** 无（用 `widget.route`）。
- **返回：** `Future<void>`。
...
- **算法：**
  ...
  2. 构建 `StatefulBuilder` 支撑、带跳类型下拉（用 `serviceHopTypeLabel` 本地化）、可选路由方法下拉（`serviceRouteMethodUiLabel`）、其 `onChanged` 也把 `endpointId` 重置为新选服务第一端点的"链接服务"下拉（`null` = 手动/自由形式跳）、条件端点下拉（只在链接服务时显示）和自由形式标签/scheme/端口/主机/路径/备注字段的 `AlertDialog`。
...
- **种类：** `_ServiceRouteEditPageState` 的方法
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 668 行）
...
- **种类：** `_ServiceRouteEditPageState` 的方法
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 683 行）
...
- **种类：** `_ServiceRouteEditPageState` 的方法
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 726 行）
...
- **种类：** 顶层函数
- **来源：** `lib/features/services/views/service_route_edit_page.dart`（第 746 行）
